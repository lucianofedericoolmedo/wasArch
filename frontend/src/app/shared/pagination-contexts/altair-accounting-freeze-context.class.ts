...
export class AltairAccountingFreezeContext extends PaginationContext<any> {
  ...
  private search(pageNumber: number, queryData: any): void {
    this.accountingService.queryAltair(pageNumber, this.pageSize, queryData).subscribe((data: PageResponse<any>) => {
      data.items.map(item => {
